...
static const uint8_t ping_sync[CMD_PING_SYNC_LEN]   = { GROUND_STICK_ID, 0x53, 0x59, 0x4e, 0x43, 0x00 };
static const uint8_t ping_ack[CMD_PING_ACK_LEN]   = { GROUND_STICK_ID, 0x41, 0x43, 0x4b, 0x00 };
...
static void commandSendGetStatus(void){
  space_packet_t packet;
  spp_tc_build_packet(&packet, SPP_GROUP_FLAG_UNSEGMENTED, SPP_SECHEAD_FLAG_NOPRESENT, 0, APID_TC_GET_STATUS, cmd_get, CMD_GET_LEN);
  radioTransmit((uint8_t*)&packet, ( SPP_PRIMARY_HEADER_LEN + packet.header.length));
  Serial.write(BINARY_START_BYTE);
...
static void commandSendGetTemp(void){
  space_packet_t packet;
  spp_tc_build_packet(&packet, SPP_GROUP_FLAG_UNSEGMENTED, SPP_SECHEAD_FLAG_NOPRESENT, 0, APID_TC_GET_TEMP, cmd_get, CMD_GET_LEN);
  radioTransmit((uint8_t*)&packet, ( SPP_PRIMARY_HEADER_LEN + packet.header.length));
  Serial.write(BINARY_START_BYTE);
...
static void commandSendGetGyro(void){
  space_packet_t packet;
  spp_tc_build_packet(&packet, SPP_GROUP_FLAG_UNSEGMENTED, SPP_SECHEAD_FLAG_NOPRESENT, 0, APID_TC_GET_GYRO, cmd_get, CMD_GET_LEN);
  radioTransmit((uint8_t*)&packet, ( SPP_PRIMARY_HEADER_LEN + packet.header.length));
  Serial.write(BINARY_START_BYTE);
...
static void commandSendGetTelemetry(void){
  space_packet_t packet;
  spp_tc_build_packet(&packet, SPP_GROUP_FLAG_UNSEGMENTED, SPP_SECHEAD_FLAG_NOPRESENT, 0, APID_TC_GET_TM, cmd_get, CMD_GET_LEN);
  radioTransmit((uint8_t*)&packet, ( SPP_PRIMARY_HEADER_LEN + packet.header.length));
  Serial.write(BINARY_START_BYTE);
