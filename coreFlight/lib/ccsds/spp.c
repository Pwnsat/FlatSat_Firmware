...
void spp_print_packet_details(const space_packet_t *packet) {
  const uint16_t version = (packet->header.identification >> 13) & 0x7;
  const uint16_t type = (packet->header.identification >> 11) & 0x1;
  const uint16_t sec_header = (packet->header.identification >> 10) & 0x1;
  const uint16_t apid = packet->header.identification & 0x7FF;

  const uint16_t seq_flags = (packet->header.sequence >> 14) & 0x3;
  const uint16_t seq_count = packet->header.sequence & 0x3FFF;

  printf("=== Space Packet Header ===");
  printf(" Version:             %u\n", version);
  printf(" Type:                %02X\n", type);
  printf(" Secondary Header:    %u\n", sec_header);
  printf(" APID:                0x%04X\n", apid);
  printf(" Sequence Flags:      0x%X (%s)\n", seq_flags,
...
int spp_unpack_packet(space_packet_t *space_packet, const uint8_t *buffer,
  ...
  if (space_packet->header.length > 0) {
