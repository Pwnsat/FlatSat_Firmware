...
static timeout_worker_t t_tm_data = {
  .inverval = 10000,
  .previous = 0
};

static timeout_worker_t t_tc_idle = {
  .inverval = 5000,
  .previous = 0
};

static timeout_worker_t t_tc_ping = {
  .inverval = 30000,
  .previous = 0
};
...
static void telemetryGetData(void){
  ...
  int ret = spp_tm_build_packet(&space_packet,
  SPP_GROUP_FLAG_UNSEGMENTED, SPP_SECHEAD_FLAG_NOPRESENT, 0,
  APID_TM_SENSORS, buffer, offset);
  ...
  radioTransmit((uint8_t*)&space_packet, ( SPP_PRIMARY_HEADER_LEN + space_packet.header.length));
}
...
void telemetryPingToGsWorker(void){
  if(millis() - t_tc_ping.previous > t_tc_ping.inverval){
    ...
    const uint8_t buffer[8] = { SPACECRAFT_ID, 0x70, 0x77, 0x6e, 0x73, 0x61, 0x74, 0x00};
    space_packet_t space_packet;
    int ret = spp_tm_build_packet(&space_packet,
    SPP_GROUP_FLAG_UNSEGMENTED, SPP_SECHEAD_FLAG_NOPRESENT, 0,
