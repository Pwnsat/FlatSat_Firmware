...
static int build_packet(spp_packet_t *space_packet, packet_type_t type,
                        grouping_flag_t flag, uint16_t apid,
                        uint16_t sequence_count, const uint8_t *payload,
                        uint16_t payload_len) {
  memset(space_packet, 0, sizeof(spp_packet_t));
  space_packet->header.packetid =
      (CCSDS_VERSION << 13) | (type << 11) | (0 << 10) |
      (apid & 0x7FF); // Secondary Header Flag = 0; By now hardcoded
  space_packet->header.packet_sequence =
      (flag << 14) | (sequence_count & 0x3FFF);
  uint16_t total_payload_len = payload_len;
  space_packet->header.length = total_payload_len;

  if (payload_len > 0 && payload != NULL) {
    if (total_payload_len > MAX_SPP_PACKET_SIZE)
...
void spp_build_tc_packet(spp_packet_t *space_packet, packet_type_t type,
                         grouping_flag_t flag, uint8_t *payload,
                         uint16_t payload_len) {
  counter_tc = (counter_tc + 1) & 0x3FFF;
  build_packet(space_packet, type, flag, APID_COMMAND, counter_tc, payload,
               payload_len);
}

void spp_build_tm_packet(spp_packet_t *space_packet, uint16_t apid,
                         uint8_t *payload, uint16_t payload_len) {
  counter_tm = (counter_tm + 1) & 0x3FFF;
  build_packet(space_packet, PACKET_TYPE_TM, GROUPING_FLAG_UNSEGMENTED, apid,
               counter_tm, payload, payload_len);
}

tc_error_t spp_parse_packet(spp_packet_t *space_packet, uint8_t *buffer,
                            uint16_t buffer_len) {
  if (buffer_len < 6) {
  ...
  uint16_t payload_len =
      space_packet->header.length + 1; // CCSDS length es (tamaño real - 1)
...
void spp_print_packet_details(spp_packet_t *packet) {
  ...
  Serial.printf(" Sequence Flags:      0x%X (%s)\n", seq_flags,
                ...
                : seq_flags == GROUPING_FLAG_CONT      ? "Continuation"
                                                       : "End");
...
void loop() {
  if (millis() - previousMillis > interval) {
    ...
    if (send_tm) {
      ...
      payload[idx++] = 0x6F;
      payload[idx++] = 0x6E;
      spp_build_tc_packet(&spp_tc, PACKET_TYPE_TC, GROUPING_FLAG_UNSEGMENTED,
                          payload, idx);
      send_tm = true;
    }

    int state =
        downlink.transmit((uint8_t *)&spp_tc, (6 + spp_tc.header.length + 1));
    if (state == RADIOLIB_ERR_NONE) {
  ...
  //   payload[idx++] = 0x03;
  //   spp_build_tc_packet(&spp_tc, PACKET_TYPE_TC, GROUPING_FLAG_UNSEGMENTED,
  //   payload, idx); send_tm = true;
  // }

  // int state = downlink.transmit((uint8_t*)&spp_tc, (6 + spp_tc.header.length
  // + 1)); if (state == RADIOLIB_ERR_NONE){
  //   Serial.print("[SYS - Radio] Transmited: ");
  //   Serial.write(payload, idx);
  //   Serial.println();
  //   // spp_print_packet_details(&spp_tc);
  ...
  if (receivedFlag) {
    ...
    if (state == RADIOLIB_ERR_NONE) {
      ...
      // spp_print_packet_details(&spp_tc);
      if (spp_tc.data[2] == 0x33 || spp_tc.data[2] == 2 ||
          spp_tc.data[3] == 0x33 || spp_tc.data[3] == 2 ||
          spp_tc.data[2] == 0x32 || spp_tc.data[3] == 0x32) {
        Serial.println("===========================================");
